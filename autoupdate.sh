...
if [ $? -eq 0 ]; then
	...
	wget https://ghproxy.com/https://dreamstart.site/images/$(date -d "@$(( $(busybox date +%s) - 86400))" +%Y-%m-%d)/$board_id$ver.img.gz -O- | gzip -dc > $board_id.img
	if [ $? -eq 0 ]; then
		wget https://https://dreamstart.site/images/$(date -d "@$(( $(busybox date +%s) - 86400))" +%Y-%m-%d)/$board_id$ver.img.md5 -O md5sum.txt
